    ...
    public enum WindowsFileError: Error, Equatable, LocalizedError {
        ...
        public var errorDescription: String? {
            switch self {
            case let .findDataError(atPath: path, code: code):
    ...
    struct WindowsFileInformation {
        ...
        init(_ findData: WIN32_FIND_DATAW) {
            creationFileTime = WindowsFileTime(findData.ftCreationTime)
            modificationFileTime = WindowsFileTime(findData.ftLastWriteTime)
            size = WindowsFileSize(findData).size
        }
    }
    ...
    struct WindowsFileTime {
        ...
        init(_ nativeTime: FILETIME) {
            ...
            seconds = UInt64(windowsTicksSince1970 / kWindowsTickPerSecond)
            nanoSeconds = UInt64(windowsTicksSince1970 % 1_000_000_000)
        }
    }
    ...
    struct WindowsFileSize {
        ...
        init(_ findData: WIN32_FIND_DATAW) {
            size = UInt64(findData.nFileSizeHigh) << 32 | UInt64(findData.nFileSizeLow)
        }
    }
